...
def pseudoColorDepth(color_map: str, depth: Tensor, near_far: tuple[float, float] = None, alpha: Tensor | None = None, interpolate: bool = False) -> Tensor:
    ...
    # apply color map
    if interpolate:
        x_coords = depth.flatten() * 2 - 1
        grid = torch.zeros((1, 1, len(x_coords), 2), dtype=depth.dtype, device=depth.device)
        grid[..., 0] = x_coords
        colormap = color_map.t()[None, :, None, :]
        depth = torch.nn.functional.grid_sample(colormap, grid, padding_mode='border').reshape(3, *depth.shape[1:])
    else:
        depth = torch.index_select(color_map, dim=0, index=(depth * 255).int().flatten()).reshape(*depth.shape[1:], 3).permute((2, 0, 1))
...
class MethodError(Framework.FrameworkError):
    """Raise in case of an exception regarding the method execution."""


class CheckpointError(Framework.FrameworkError):
    """Raise in case of an exception regarding a checkpoint."""


class RendererError(Framework.FrameworkError):
    """Raise in case of an exception regarding a renderer."""


class ModelError(Framework.FrameworkError):
    """Raise in case of an exception regarding a model."""


class TrainingError(Framework.FrameworkError):
    """Raise in case of an exception regarding training."""
